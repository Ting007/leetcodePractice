import math
class Solution:
    def median(self, total, k):
        if len(total)%2 ==0:
            return float(total[k] + total[k-1])/2.0
        else:
            return float(total[k])

    def compare(self, nums1, n, i, nums2, m, j, k):
        """
        nums1 and nums2 are two array of nums
        n is the current location of nums1
        m is the current location of nums2
        i is the cutting site of nums1
        j is the cutting site of nums2
        k = i + j the index of median value
        rtype: [true/false, median value]
        """
        # here the n is always no less than m and k
        #print(n, i, m, j, k)
        if i >= len(nums1):
            n_left = nums1[len(nums1)-1]
            n_right = max(nums1[len(nums1)-1], nums2[len(nums2)-1]) +1
            m_right = nums2[j]
            m_left = nums2[j-1]
            ...
            n_right = nums1[i]  
            n_left = nums1[i-1]
            m_right = max(nums1[len(nums1)-1], nums2[len(nums2)-1]) +1
            m_left = nums2[len(nums2)-1]
        elif i <= 0:
            n_right = nums1[i]
            n_left = min(nums1[0], nums2[0]) -1
            m_right = nums2[j]
            m_left = nums2[j-1]
        elif j <= 0:
            n_left = nums1[i-1]
            n_right = nums1[i]
            m_right = nums2[j]
            m_left = min(nums1[0], nums2[0]) -1


        else: # the condition should be n > k
            n_left = nums1[i-1]
            n_right = nums1[i]
            m_left = nums2[j-1]
            m_right = nums2[j]
        ...
        if n_left <= m_right and n_right >= m_left:
            ...
            print(n_left, m_left, n_right, m_right)
            if (len(nums1)+len(nums2))%2 == 1: 
                median = float(min(n_right, m_right))
            else:
                median = float(max(n_left, m_left) + min(n_right, m_right))/2.0
            ...
            dist = abs(n - i)
            n = i
            i = i- math.floor(dist/2)
            j = k-i
            return self.compare(nums1, n, i, nums2, m, j, k)
        elif n_right <= m_left:
            #print("smaller")
            dist = abs(n-i)
            n = i
            i = math.floor(dist/2)+i
            j = k-i
            return self.compare(nums1, n, i, nums2, m, j, k)
    ...
    def findMedianSortedArrays(self, nums1, nums2):
        ...
        if n ==0 or m == 0:
            total = nums2+ nums1
            median = self.median(total, k)
            return median

        elif nums1[0] >= nums2[m-1]:
            total = nums2+ nums1
            median = self.median(total, k)
            return median
        elif nums1[n-1] <= nums2[0]:
            total = nums1 + nums2
            median = self.median(total, k)
            return median
        if n >= m:
            i = k
            j = 0
            return self.compare(nums1, 0, i, nums2, 0, j, k)
        else:
            j = k
            i = 0
            return self.compare(nums2, 0, j, nums1, 0, i, k)
...
def main():
        foo = Solution()
        nums1 = [2, 4, 6, 8, 50, 64, 79, 90] 
        nums2 = [1, 3, 5, 88, 345, 801]
